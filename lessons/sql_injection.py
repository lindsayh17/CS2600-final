...
def create_search_query(account_id: int, search_term: str) -> str:
    ...
    #search for a " character in the sql injection
    index = search_term.find("\"")
    #keep the string the same if the double quote was not found
    if index == -1:
        sanitized_search_term = search_term
    else:
        sanitized_search_term = "null"
